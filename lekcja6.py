...
def findApproximateValue(value, percentRange = 10):
    lowestValule = value - (percentRange / 100) * value
    highestValule = value + (percentRange / 100) * value
    return random.randint(int(lowestValule), int(highestValule))
...
while gameLenght > 0:
    ...
    if(gamerAnswer == "yes"):
        ...
        if drawEvent == Event.Chest:
            ...
            gamerReward = findApproximateValue(rewardForChests[drawColor])
